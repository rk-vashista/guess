...
def main():
    ...
    start_minimized = args.minimize or config.get("ui", {}).get("start_minimized", False)
    
    if start_minimized and config.get("ui", {}).get("minimize_to_tray", True):
        logging.info("Starting minimized to tray")
        # Don't show the main window, just initialize the tray icon
        main_window.setup_tray_icon()
    else:
        main_window.show()
